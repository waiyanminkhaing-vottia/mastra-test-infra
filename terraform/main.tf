...
resource "aws_lightsail_instance" "mastra_instance" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "# Backup and modify nginx.conf to remove default server block",
      "sudo cp /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup",
      "sudo awk '/^[[:space:]]*server[[:space:]]*{/{flag=1} flag && /^[[:space:]]*}/{flag=0; next} !flag' /etc/nginx/nginx.conf > /tmp/nginx_new.conf",
      "sudo mv /tmp/nginx_new.conf /etc/nginx/nginx.conf",

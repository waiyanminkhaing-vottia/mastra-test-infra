...
resource "aws_lightsail_instance" "mastra_instance" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "# Backup original nginx.conf and create clean version without server blocks",
      "sudo cp /etc/nginx/nginx.conf /etc/nginx/nginx.conf.backup",
      "sudo grep -v -E '^[[:space:]]*(server|location|root|error_page)[[:space:]]*' /etc/nginx/nginx.conf | sudo grep -v -E '^[[:space:]]*}[[:space:]]*$' > /tmp/nginx_clean.conf",
      "echo '}' | sudo tee -a /tmp/nginx_clean.conf > /dev/null",
      "sudo mv /tmp/nginx_clean.conf /etc/nginx/nginx.conf",
